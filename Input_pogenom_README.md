...
The easiest and recommended way to install this pipeline is through conda in an isolated environment. Below an example of how to set up the pipeline (including its required software) in an environment:
...
Pfam database can be downloaded at:
...
A)	01_INDEXING
 The indexed genome file(s) are stored in this directory. A subdirectory per dataset and genome is created.  Example of filename:  01_INDEXING/<dataset>/<genome_name>/
...
Where <parameters> is a string of the key parameters used, for instance 'cov_10_bdth_40_mpq_20_bq_15'. The corresponding log file for these steps is(are):
...
Example of filename:    06_VCF/<dataset>/params_<parameters>/<genome_name>.vcf
