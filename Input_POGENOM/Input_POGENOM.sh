...
err_report() {
    ...
    mess2="Use the command:\n    'snakemake -s snakefiles/steps_on_all_samples--unlock'"
    mess3="Use the command:\n    'snakemake -s snakefiles/steps_on_selected_samples --config my_mag='"$mag"' my_samples='"$samples"' --unlock'"
    mess="or\n    'snakemake -s snakefiles/step2_calling_several_BAMs --config mag_name='"$mag"' --unlock' or\n    'snakemake -s snakefiles/step2_B_calling_one_BAM --config mag_name='"$mag"' --unlock'"
...
if [ "$annotation" == yes ]; then
    if [[ "$pfam_db_path" != /* ]] || ! test -s $pfam_db_path; then echo "Please provide an absoltute path to the Pfam database in configfile"; rm temporal; exit 1; fi
...
if  [[ "$mode" == prefilt ]]; then
         ...
         echo "INFO: Generating Reads subsets from dataset $dataset - Fraction used $fraction"
         bash src/create_prefilt_Reads_subdir.sh $fraction $genomes_ext $reads_ext $temp_sub_Reads_dir $dataset
         echo "INFO: Calculating Genome Median coverage - sub-samples from dataset $dataset - Median coverage threshold $min_coverage"
         snakemake -s snakefiles/step_sample_pre_screening -j $threads $extra_params 2>log_files/samples_filter_$dataset.log
         ...
         file_empty=$(grep -v "#" $result_dir/Selected_samples_Genomes.txt | wc -l)  # File generated by snakefiles/step_filter (line 88) and contains the list of Genomes and selected samples
             if [ "$file_empty" -eq 0 ]; then
                ...
                do # Mapping and coverage/breadth calculations
                  ...
                  snakemake -s snakefiles/steps_on_selected_samples step1_all --config my_mag="$mag" my_samples="$samples" -j $threads $extra_params 2> log_files/$dataset.$mag.coverage_breadth.log
                  echo "INFO: Generating VCF files - Genome $mag"
                  snakemake -s snakefiles/steps_on_selected_samples vcf --config my_mag="$mag" my_samples="$samples" -j $threads $extra_params 2> log_files/$dataset.$mag"_vcf_files.log"
                  if [ "$annotation" == yes ]; then
...
cd $workdir
    echo "INFO: Calculating Genome Median coverage and breadth - Dataset: $dataset - Median coverage threshold: $min_coverage - Breadth threshold: $min_breadth %"
    snakemake -s snakefiles/steps_on_all_samples step1_all -j $threads $extra_params 2> log_files/$dataset"_Genomes_coverage_breadth.log"
    echo "INFO: Generating VCF files - Dataset $dataset"
    snakemake -s snakefiles/steps_on_all_samples vcf -j $threads $extra_params 2> log_files/$dataset"_Genomes_vcf_files.log"
    if [ "$annotation" == yes ]; then
