...
if [ "$(ls -A $wdir/$MAGs_path/$i)" ] # if directory is not empty
  then
    ...
    if (( $(echo "${#files[@]} > 1" | bc -l) )) # if there are more than 1 bam file
       then
          snakemake -s snakefiles/step2_calling_several_BAMs --config mag_name="$i" -j $threads -F --quiet  # It merges BAM files and generates VCF files

    elif (( $(echo "${#files[@]} == 1" | bc -l) ))
        then
          snakemake -s snakefiles/step2_B_calling_one_BAM --config mag_name="$i" -j $threads -F --quiet  # It generates VCF files
    fi
else
